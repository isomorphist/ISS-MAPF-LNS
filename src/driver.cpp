...
int main(int argc, char** argv)
{
	...
	desc.add_options()
        ...
        //("map,m", po::value<string>()->default_value("../mapf_instances/mapf-map/random-32-32-20.map"), "input file for map")
		//("agents,a", po::value<string>()->default_value("../mapf_instances/mapf-scen-random/scen-random/random-32-32-20-random-2.scen"), "input file for agents")
		("map,m", po::value<string>()->default_value("../mapf_instances/mapf-map/empty-8-8.map"), "input file for map")
        ("agents,a", po::value<string>()->default_value("../mapf_instances/mapf-scen-random/scen-random/empty-8-8-random-1.scen"), "input file for agents")
		//("agents,a", po::value<string>()->default_value("../machine_learning/lambdarank/train_instances/test/train-001.scen"), "input file for agents")
        ("agentNum,k", po::value<int>()->default_value(12), "number of agents")
        ...
        ("modus", po::value<string>()->default_value("generateTrainData"),
                "Modus (generateTrainData, testML, test)")
        ("pathTrainData", po::value<string>()->default_value("../machine_learning/lambdarank/train_data/test"),
                "Path to training data")
        ("pathResults", po::value<string>()->default_value("../machine_learning/lambdarank/results/test"),
                "Path to results")
        ("numInitialSolutions", po::value<int>()->default_value(3), "Number of initial solutions to generate")
        ("numLNSRuns", po::value<int>()->default_value(1), 
                "Number of LNS runs per initial solution for generating training data")
		;
	po::variables_map vm;
    ...
    string modus = vm["modus"].as<string>();
    bool succ = false;
	...
	if (vm["solver"].as<string>() == "LNS")
    {
        LNS lns(instance, time_limit,
            ...
            vm["sipp"].as<bool>(),
            vm["pathResults"].as<string>(),
            vm["pathTrainData"].as<string>(),
            vm["numInitialSolutions"].as<int>(),
            vm["numLNSRuns"].as<int>(),
            screen, pipp_option);

        if (modus == "generateTrainData")
        {
            lns.generateTrainData();
        }
        else
        {
            assert(vm["numLNSRuns"].as<int>() == 1);
            if (modus == "testML")
                bool succ = lns.runML();
            else if (modus == "test")
                bool succ = lns.run();
